use manteau_core::{
  ...
  templating::attributes::{
    measurements::{Em, PositiveFinite, Rem},
    urls::ImageUrl,
  },
};
...
fn imported_bodies_preserve_the_renderer_character_contract() {
  assert!(matches!(
    Rendered::new("", ""),
    Err(RenderedBodyError::Empty)
  ));
  assert!(matches!(
    Rendered::new("<p>hello</p>\0", "hello"),
    Err(RenderedBodyError::Html(_))
  ));
  assert!(matches!(
    Rendered::new("<p>hello</p>", "hello\0"),
    Err(RenderedBodyError::Plaintext(_))
  ));
  assert!(
    serde_json::from_str::<Rendered>(
      r#"{"html":"<p>hello</p>","text":"hello\u0000"}"#
    )
    .is_err()
  );
}
...
fn markup_values_reject_unsupported_links_and_nonfinite_dimensions() {
  ...
  assert!(
    !format!("{:?}", Url::try_parse(private).unwrap())
      .contains("private-token")
  );
  assert!(
    !format!(
      "{:?}",
      Url::try_parse("javascript:private-token").unwrap_err()
    )
    .contains("private-token")
  );
