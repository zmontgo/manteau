...
impl MailjetErrorKind {
    ...
    pub(crate) fn err(
        ...
        MailjetError { kind: self, source: source.into() }
    }
}
...
impl TransportFailure for MailjetError {
    ...
    fn is_auth(&self) -> bool {
        matches!(self.kind, MailjetErrorKind::Auth)
    }
...
impl Transport for MailjetTransport {
    ...
    async fn send(&self, message: &Message) -> Result<MailjetReceipt, MailjetError> {
        ...
        if !status.is_success() {
            ...
            let rejection = ProviderRejection { status: status_u16, body };
