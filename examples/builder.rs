...
//!   - you need to assemble a template from values that are inconvenient
//!     to splice through `{expr}` (long generic chains, async-awaited
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let body = Body::new()
    ...
    .push(
      Section::new()
        .padding(PaddingOptions::new().t(Pixels::new(32).into()).b(Pixels::new(24).into()))
